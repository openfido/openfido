...
def test_workflow_pipeline(app, workflow, pipeline):
    ...
    a_to_b = WorkflowPipelineDependency(
        source_workflow_pipeline=pipeline_a,
        dest_workflow_pipeline=pipeline_b,
    )
    pipeline_a.source_workflow_pipelines.append(a_to_b)
    b_to_c = WorkflowPipelineDependency(
        source_workflow_pipeline=pipeline_b,
        dest_workflow_pipeline=pipeline_c,
    )
    pipeline_b.source_workflow_pipelines.append(b_to_c)
    db.session.commit()
