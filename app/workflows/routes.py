...
@organization_workflow_bp.route(
    "/<organization_uuid>/workflows/<workflow_uuid>", methods=["GET"]
)
@any_application_required
@validate_organization()
def workflow(organization_uuid, workflow_uuid):
    ...
    """
    try:
        return jsonify(fetch_workflow(organization_uuid, workflow_uuid))
    except HTTPError as http_error:
        return {"message": http_error.args[0]}, 503
    except ValueError as value_error:
        return jsonify(value_error.args[0]), 400
...
@organization_workflow_bp.route(
    "/<organization_uuid>/workflows/<workflow_uuid>", methods=["PUT"]
)
@any_application_required
@validate_organization()
def workflow_update(organization_uuid, workflow_uuid):
    ...
    """
    try:
        return jsonify(update_workflow(organization_uuid, workflow_uuid, request.json))
    except HTTPError as http_error:
        return {"message": http_error.args[0]}, 503
    except ValueError as value_error:
        return jsonify(value_error.args[0]), 400
